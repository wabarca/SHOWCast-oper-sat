...
grid = remap(
    path_proc, variable, extent, resolution, h, a, b, longitude, x1, y1, x2, y2
)
...
#     "dpi": 150,
#     "states_color": 'gold', "states_width": data.shape[0] * 0.002,
#     "countries_color": 'gold', "countries_width": data.shape[0] * 0.002,
#     "continents_color": 'gold', "continents_width": data.shape[0] * 0.002,
#     "grid_color": 'white', "grid_width": data.shape[0] * 0.00125, "grid_interval": 1.0,
#     "vmin": vmin, "vmax": vmax, "cmap": cmap,
#     "title_text": "GOES-" + satellite[1:3] + " Band " + band, "title_size": int(data.shape[1] * 0.015), "title_x_offset": int(data.shape[1] * 0.01), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016),
#     "thick_interval": thick_interval, "cbar_labelsize": int(data.shape[0] * 0.025), "cbar_labelpad": -int(data.shape[0] * 0),
...
#     "resolution": band_resolution_km,
#     "dpi": 150,
#     "states_color": 'gold', "states_width": data.shape[0] * 0.004,
#     "countries_color": 'gold', "countries_width": data.shape[0] * 0.004,
#     "continents_color": 'gold', "continents_width": data.shape[0] * 0.004,
#     "grid_color": 'white', "grid_width": data.shape[0] * 0.0025, "grid_interval": 1.0,
#     "vmin": vmin, "vmax": vmax, "cmap": cmap,
#     "title_text": "GOES-" + satellite[1:3] + " Band " + band, "title_size": int(data.shape[1] * 0.03), "title_x_offset": int(data.shape[1] * 0.02), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016),
#     "thick_interval": thick_interval, "cbar_labelsize": int(data.shape[0] * 0.05), "cbar_labelpad": -int(data.shape[0] * 0),
#     "file_name_id_1": satellite,  "file_name_id_2": product
#     }
plot_config = {
    "resolution": band_resolution_km,
    ...
    "states_color": "gold",
    "states_width": 0.5,
    "countries_color": "gold",
    "countries_width": 0.5,
    "continents_color": "gold",
    ...
    + " $\mu{m}$)",
    "title_size": 8,
    "title_x_offset": int(data.shape[1] * 0.01),
    "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016),
    "thick_interval": thick_interval,
    "cbar_labelsize": 7,
    "cbar_labelpad": -int(data.shape[0] * 0),
    "file_name_id_1": satellite,
    "file_name_id_2": product,
}
# ---------------------------------------------------------------------------------------------
# ---------------------------------------------------------------------------------------------
...
if sector == "_SLV" or sector == "_EGH":
    ...
    ax.add_geometries(
        shapefile,
        ccrs.PlateCarree(),
        edgecolor=plot_config["states_color"],
        facecolor="none",
    ...
    ax.add_geometries(
        shapefile,
        ccrs.PlateCarree(),
        edgecolor=plot_config["countries_color"],
        facecolor="none",
    ...
    ax.add_geometries(
        shapefile,
        ccrs.PlateCarree(),
        edgecolor=plot_config["continents_color"],
        facecolor="none",
...
# gl.set_path_effects([PathEffects.withStroke(linewidth=0.25, foreground='b')])
...
if sector == "_MON" and int(band) == 2:
    plt.annotate(
        ...
        fontweight="bold",
        color="white",
        bbox=dict(boxstyle="square,pad=2", facecolor="darkblue", edgecolor="none"),
        zorder=6,
    )
...
for each_section in conf.sections():
    for each_key, each_val in conf.items(each_section):
        if each_key == "label":
            labels.append(each_val)
        if each_key == "lon":
            city_lons.append(float(each_val))
        if each_key == "lat":
            city_lats.append(float(each_val))
        if each_key == "x_offset":
            x_offsets.append(float(each_val))
        if each_key == "y_offset":
            y_offsets.append(float(each_val))
        if each_key == "size":
            sizes.append(int(each_val))
        if each_key == "color":
            colors.append(each_val)
        if each_key == "marker_type":
            marker_types.append(each_val)
        if each_key == "marker_color":
            marker_colors.append(each_val)
        if each_key == "marker_size":
            marker_sizes.append(each_val)

import matplotlib.patheffects as PathEffects

for label, xpt, ypt, x_offset, y_offset, size, col, mtype, mcolor, msize in zip(
    labels,
    city_lons,
    city_lats,
    x_offsets,
...
cb.ax.tick_params(
    axis="y", width=0.5, colors="yellow", labelsize=plot_config["cbar_labelsize"]
)
# ---------------------------------------------------------------------------------------------
# ---------------------------------------------------------------------------------------------
...
# driver.CreateCopy(out_dir + plot_config["file_name_id_1"] + "_" + plot_config["file_name_id_2"] + "_" + date_file + '.tif',grid,0)
...
# os.remove(out_dir + plot_config["file_name_id_1"] + "_" + plot_config["file_name_id_2"] + "_" + date_file + '.webp')
