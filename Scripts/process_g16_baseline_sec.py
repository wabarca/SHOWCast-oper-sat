...
if (variable == 'HT'):
    ...
    prod_name = "Altura de tope de nube"
elif (variable == 'TEMP'):
    # ACHTF
    cmap = 'jet'
    vmin = -95
    ...
    data[data == max(data[0])] = np.nan
    data[data == min(data[0])] = np.nan
    prod_name = "Temperatura de tope de nube"
elif (variable == 'BCM'):
    # ACMF
    colors = ["#ffffff"]
    ...
    data[data == max(data[0])] = np.nan
    data[data == min(data[0])] = np.nan
    prod_name = "Profundidad Óptica de Aerosoles"
elif (variable == 'COD'):
    # CODF
    cmap = 'jet'
    vmin = 0
    ...
    data -= 273.15
    data[data == min(data[0])] = np.nan
    prod_name = "Temperatura de Superficie Terrestre"
elif (variable == 'RRQPE'):
    # RRQPE
    ...
    data[data == min(data[0])] = np.nan
    data[data < 6] = np.nan
    prod_name = "Intensidad de lluvia QPE"
elif (variable == 'SST'):
    # SSTF
    cmap = 'jet'
    # vmin = -5.15
    ...
    prod_name = "Temperatura de Superficie del Mar"
elif (variable == 'TPW'):
    # TPW
    ...
    data[data == max(data[0])] = np.nan
    data[data == min(data[0])] = np.nan
    prod_name = "Agua Precipitable Total"
#------------------------------------------------------------------------------------------------------
# Plot configuration
plot_config = {
...
"title_text": "GOES-19 " + prod_name + " [" + variable_units + "]", "title_size": 8, "title_x_offset": int(data.shape[1] * 0.01), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016),
...
if (variable == 'RRQPE'):
    ...
    img = ax.imshow(data, vmin=plot_config["vmin"], vmax=plot_config["vmax"], origin='upper', extent=img_extent, cmap=plot_config["cmap"], alpha=0.75, zorder=2)
else:
    img = ax.imshow(data, vmin=plot_config["vmin"], vmax=plot_config["vmax"], origin='upper', extent=img_extent, cmap=plot_config["cmap"], alpha=0.75, zorder=2)
...
if sector == "_SLV":
    ...
    ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["countries_color"],facecolor='none', linewidth=plot_config["countries_width"], zorder=4)
     ...
     # Add states and provinces
    shapefile = list(shpreader.Reader(main_dir + '//Shapefiles//Municipios.shp').geometries())
    ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["states_color"],facecolor='none', linewidth=plot_config["states_width"], zorder=3)
...
# newax = fig.add_axes([0.89, 0.015, 0.10, 0.10], anchor='SW', zorder=9) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
...
if (variable == 'Phase'):
	...
	newax = fig.add_axes([0.87, 0.87, 0.12, 0.12], anchor='NE', zorder=10) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
