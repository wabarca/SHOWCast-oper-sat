...
fig = plt.figure(
    figsize=(1920 / float(plot_config["dpi"]), 1163 / float(plot_config["dpi"])),
    dpi=plot_config["dpi"],
)
...
urllib.request.urlretrieve(
    nhc_gis_files, main_dir + "//Shapefiles//nhc//gtwo_shapefiles.zip"
)
with zipfile.ZipFile(
    main_dir + "//Shapefiles//nhc//gtwo_shapefiles.zip", "r"
) as zip_ref:
...
areas_low = [
    x.geometry
    for x in shapefile
    if float(x.attributes["PROB7DAY"].replace("%", "")) < 40
]
areas_medium = [
    x.geometry
    for x in shapefile
    if float(x.attributes["PROB7DAY"].replace("%", "")) >= 40
    and float(x.attributes["PROB7DAY"].replace("%", "")) <= 60
]
areas_high = [
    x.geometry
    for x in shapefile
    if float(x.attributes["PROB7DAY"].replace("%", "")) > 60
]
...
ax.add_geometries(
    ...
    facecolor="yellow",
    alpha=0.4,
    linewidth=plot_config["states_width"] * 6,
    zorder=4,
)
ax.add_geometries(
    areas_medium,
...
points_low = [
    x for x in shapefile if float(x.attributes["PROB7DAY"].replace("%", "")) < 40
]
points_medium = [
    x
    for x in shapefile
...
points_high = [
    x for x in shapefile if float(x.attributes["PROB7DAY"].replace("%", "")) > 60
]
...
for pl in points_low:
    ...
    if float(pl.attributes["PROB2DAY"].replace("%", "")) < 40:
        ...
        float(pl.attributes["PROB2DAY"].replace("%", "")) >= 40
        and float(pl.attributes["PROB2DAY"].replace("%", "")) <= 60
    ):
        text = ax.text(
            pl.geometry.x + 0.5,
            pl.geometry.y - 0.5,
...
for pm in points_medium:
    ...
    if float(pm.attributes["PROB2DAY"].replace("%", "")) < 40:
        ...
        float(pm.attributes["PROB2DAY"].replace("%", "")) >= 40
        and float(pm.attributes["PROB2DAY"].replace("%", "")) <= 60
    ):
        text = ax.text(
            pm.geometry.x + 0.5,
            pm.geometry.y - 0.5,
...
for ph in points_high:
    ...
    if float(ph.attributes["PROB2DAY"].replace("%", "")) < 40:
        ...
        float(ph.attributes["PROB2DAY"].replace("%", "")) >= 40
        and float(ph.attributes["PROB2DAY"].replace("%", "")) <= 60
    ):
        text = ax.text(
            ph.geometry.x + 0.5,
            ph.geometry.y - 0.5,
...
for name in tsystems:
    ...
    if storm.invest == True:
        ...
        urllib.request.urlretrieve(
            nhc_gis_files,
            main_dir + "//Shapefiles//nhc//" + name.lower() + "_5day_latest.zip",
        )
        with zipfile.ZipFile(
            main_dir + "//Shapefiles//nhc//" + name.lower() + "_5day_latest.zip", "r"
        ) as zip_ref:
            zip_ref.extractall(main_dir + "//Shapefiles//nhc//")
        # Reading NHC shapefiles
        shp_files = []
        for filename in glob.glob(
            main_dir + "//Shapefiles//nhc//" + name.lower() + "*_5day_lin.shp"
        ):
            shp_files.append(os.path.normpath(filename))
        for filename in glob.glob(
            main_dir + "//Shapefiles//nhc//" + name.lower() + "*_5day_pgn.shp"
        ):
            shp_files.append(os.path.normpath(filename))
        for filename in glob.glob(
            main_dir + "//Shapefiles//nhc//" + name.lower() + "*_5day_pts.shp"
        ):
            shp_files.append(os.path.normpath(filename))
        # Add line trajectory
        ...
        ax.add_geometries(
            ...
            edgecolor="white",
            facecolor="none",
            linewidth=plot_config["states_width"] * 4,
            zorder=6,
        )
        ...
        # Plotting positions and labels
        ax.scatter(
            [shapefile.x for shapefile in shapefile],
            [shapefile.y for shapefile in shapefile],
            edgecolor="black",
            facecolor="red",
            transform=ccrs.PlateCarree(),
            zorder=8,
        ...
        for field in shapefile:
            text = ax.text(
                (field.attributes["LON"] + 0.5),
                (field.attributes["LAT"] + 0.5),
                field.attributes["DVLBL"],
                color="blue",
                fontsize="large",
                fontweight="bold",
                zorder=9,
...
gl = ax.gridlines(
    ...
    xlocs=np.arange(-180, 180, plot_config["grid_interval"]),
    ylocs=np.arange(-180, 180, plot_config["grid_interval"]),
    draw_labels=True,
    zorder=10,
)
...
newax = fig.add_axes(
    [0.9, 0.0, 0.10, 0.10], anchor="S", zorder=14
...
newax = fig.add_axes(
    [0.0, 0.0, 0.15, 0.15], anchor="S", zorder=14
)  #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
newax.imshow(my_logo)
newax.axis("off")
