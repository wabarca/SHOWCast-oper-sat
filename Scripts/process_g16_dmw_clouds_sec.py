...
# Read the image
path_dmw = (sys.argv[1])
#path_dmw = ("..//Samples//OR_ABI-L2-DMWF-M6C14_G19_s20192970600363_e20192970610071_c20192970623560.nc")
...
#path_ch14 = re.sub('OR_ABI-L2-DMWF*(.*)', '', path_dmw)

# Get the Band 14, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch14 = re.sub('DMWF-C14\\\\OR_ABI-L2-DMWF*(.*)', '', path_dmw)
    path_ch14 = re.sub('GOES-R-Level-2-Products', '', path_ch14)
else:
    path_ch14 = re.sub('DMWF-C14/OR_ABI-L2-DMWF*(.*)', '', path_dmw)
    path_ch14 = re.sub('GOES-R-Level-2-Products', '', path_ch14)
...
"dpi": 150,
"states_color": 'gold', "states_width": 0.3,
"countries_color": 'gold', "countries_width": 0.3,
"continents_color": 'gold', "continents_width": 0.3,
"grid_color": 'white', "grid_width": 0.3, "grid_interval": 5.0,
"title_text": "GOES-19 Vientos de movimiento derivados + Banda 14 ", "title_size": 8, "title_x_offset": int(data.shape[1] * 0.01), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016),
...
for x in range(1, 7):
    ...
    # wind_speed[wind_speed<10] = np.nan
    ...
    # Make the barb plot
    # ax.barbs(lons, lats, u_comp, v_comp, length = 4, sizes = dict(emptybarb=0.25, spacing=0.5, height=0.5), linewidth=0.25, pivot='tip', barbcolor=color, transform=ccrs.PlateCarree(),antialiased=True, zorder=5)
    # Make the barb plot
    n = 3
    ax.barbs(lons[::n], lats[::n], u_comp[::n], v_comp[::n], length = 4, linewidth=0.25, barbcolor=color, antialiased=True, transform=ccrs.PlateCarree(), zorder=5)
...
gl = ax.gridlines(color=plot_config["grid_color"], alpha=0.5, linestyle='--', linewidth=plot_config["grid_width"], xlocs=np.arange(-180, 180, plot_config["grid_interval"]), ylocs=np.arange(-180, 180, plot_config["grid_interval"]), draw_labels=True, zorder=8)
...
# Add a title
plt.annotate(plot_config["title_text"] + " " + date_formated , xy=(0.015,0.95), xycoords='figure fraction', fontsize=plot_config["title_size"], fontweight='bold', color='white', bbox=dict(boxstyle="round",fc=(0.0, 0.0, 0.0), ec=(1., 1., 1.)), zorder=9)
...
newax = fig.add_axes([0.908, 0.0, 0.10, 0.10], anchor='S', zorder=12) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
newax.imshow(my_logo)
newax.axis('off')

# Add a legend to the plot
my_legend = plt.imread(main_dir + '//Legends//DMWF_legend.png')
newax = fig.add_axes([0.83, 0.8, 0.15, 0.15], anchor='NE', zorder=13) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
