	...
	for i in range(len(lon)):
		...
		if (extent_user[0] <= lon[i] <= extent_user[2]):
			if (extent_user[1] <= lat[i] <= extent_user[3]):
				...
				plot_skewt(sound, ax=ax, title=titulo,lift_parcel=True, plot_winds=False, diags=["CAPE", ],label_altitudes=True)
				...
				im = plt.imread(main_dir + '//Logos//my_logo.webp')
				...
				# Save fig
				plt.savefig(out_dir + 'N20_NUCAPS_SLV_' + tempo[i].strftime('%Y%m%d%H%M%S') + '_' + str(i).zfill(2) + '.png'.format(i), bbox_inches='tight', pad_inches=0)

				# Convert to webp
				from PIL.WebPImagePlugin import Image          
				im = Image.open(out_dir + 'N20_NUCAPS_SLV_' + tempo[i].strftime('%Y%m%d%H%M%S') + '_' + str(i).zfill(2) + '.png')
				im.save(out_dir + 'N20_NUCAPS_SLV_' + tempo[i].strftime('%Y%m%d%H%M%S') + '_' + str(i).zfill(2) + '.webp', format = "WebP", lossless = True)
				im.close()
				# Delete aux files
				os.remove(out_dir + 'N20_NUCAPS_SLV_' + tempo[i].strftime('%Y%m%d%H%M%S') + '_' + str(i).zfill(2) + '.png')  
