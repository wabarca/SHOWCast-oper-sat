...
if (seconds == '00'):
    ...
    if (int(minutes) % acum_minutes == 0):
        ...
        heatmap = gaussian_filter(heatmap, sigma=0.75)
        heatmap = heatmap.T
        ...
        # if osystem == "Windows":
        #     path_ch14 = re.sub('GOES-R-GLM-Products\\\\OR_GLM*(.*)', '', path_glm)
        # else:
        #     path_ch14 = re.sub('GOES-R-GLM-Products/OR_GLM*(.*)', '', path_glm)
        # print(path_ch14)

        # file = []
        # for filename in sorted(glob.glob(path_ch14+'GOES-R-CMI-Imagery//Band14//OR_ABI-L2-CMIPF-M*C14_G16*.nc')):
        ...
        # Read the central longitude
        # longitude = file_ch14.variables['goes_imager_projection'].longitude_of_projection_origin

        # Read the semi major axis
        # a = file_ch14.variables['goes_imager_projection'].semi_major_axis

        # Read the semi minor axis
        # b = file_ch14.variables['goes_imager_projection'].semi_minor_axis

        # Calculate the image extent
        # h = file_ch14.variables['goes_imager_projection'].perspective_point_height
        # x1 = file_ch14.variables['x_image_bounds'][0] * h
        # x2 = file_ch14.variables['x_image_bounds'][1] * h
        # y1 = file_ch14.variables['y_image_bounds'][1] * h
        # y2 = file_ch14.variables['y_image_bounds'][0] * h

        # Choose the visualization extent (min lon, min lat, max lon, max lat)
        # extent = [float(sys.argv[2]), float(sys.argv[3]), float(sys.argv[4]), float(sys.argv[5])]

        # # Read the band number
        # band = str(file_ch14.variables['band_id'][0]).zfill(2)
        ...
        "grid_color": 'white', "grid_width": 0.3, "grid_interval": 1.0,
        "title_text": "GOES-19 GLM Densidad de grupos ("+str(acum_minutes)+" min) + Banda ", "title_size": 8, "title_x_offset": int(sizex * 0.01), "title_y_offset": sizey - int(sizey * 0.016),
        ...
        fig = plt.figure(figsize=(1920/float(plot_config["dpi"]), 1587/float(plot_config["dpi"])), dpi=plot_config["dpi"])
        ...
        ax.set_extent([extent[0], extent[2], extent[1], extent[3]], crs=ccrs.Mercator())
        ...
        # glm = ax.imshow(heatmap, vmin=0, vmax=60, interpolation='bilinear', extent=extent_glm, origin='lower', cmap=my_cmap, transform=proj, alpha = 0.75, zorder=4)
         # Plot the accumulated flashes, groups and events
        # img2 = ax.plot(e_lons,e_lats, 'o', markersize=2.0, color='#fffb8a', alpha = 0.01, transform=proj, zorder=5)
        # img3 = ax.plot(g_lons,g_lats, '.', markersize=3.0, color='yellow', alpha = 1.0, transform=proj, zorder=5)
        # img4 = ax.scatter(g_lons,g_lats, marker='$\U0001F5F2$', s=150, edgecolor='black', facecolor='orange', transform=proj, zorder=9)
        img4 = ax.scatter(g_lons,g_lats, marker='o', s=20, edgecolor='black', facecolor='orange', transform=proj, zorder=9)
        # img4 = ax.plot(f_lons,f_lats, 'o', markersize=2.0, color='#fff700', alpha = 0.20, transform=proj, zorder=7)
        #img2 = ax.plot(e_lons,e_lats, 'o', markersize=1.0, color='#dedede', alpha = 0.01, transform=proj, zorder=5)
        #img3 = ax.plot(g_lons,g_lats, 'o', markersize=1.0, color='#ededed', alpha = 0.02, transform=proj, zorder=6)
        #img4 = ax.plot(f_lons,f_lats, 'o', markersize=1.0, color='#ffffff', alpha = 0.20, transform=proj, zorder=7)
        #---------------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------------
        # Add states and provinces
        # shapefile = list(shpreader.Reader(main_dir + '//Shapefiles//El_Salvador_departamentos.shp').geometries())
        # ax.add_geometries(shapefile, proj, edgecolor=plot_config["states_color"],alpha = 0.3,facecolor='none', linewidth=plot_config["states_width"], zorder=6)

        # Add countries
        # shapefile = list(shpreader.Reader(main_dir + '//Shapefiles//Centro_America.shp').geometries())
        # ax.add_geometries(shapefile, proj, edgecolor=plot_config["countries_color"],alpha = 0.3,facecolor='none', linewidth=plot_config["countries_width"], zorder=7)

        # Add continents
        # shapefile = list(shpreader.Reader(main_dir + '//Shapefiles//GSHHS_h_L1_clipped.shp').geometries())
        # ax.add_geometries(shapefile, proj, edgecolor=plot_config["continents_color"],facecolor='none', linewidth=plot_config["continents_width"], zorder=10)

        # Add gridlines
        # gl = ax.gridlines(color=plot_config["grid_color"], alpha=0.3, linestyle='--', linewidth=plot_config["grid_width"], xlocs=np.arange(-180, 180, plot_config["grid_interval"]), ylocs=np.arange(-180, 180, plot_config["grid_interval"]), draw_labels=True, zorder=8)
        ...
        # Add a title
        # plt.annotate(plot_config["title_text"] + " " + date_formated, xy=(0.015,0.965), xycoords='figure fraction', fontsize=plot_config["title_size"], fontweight='bold', color='white', bbox=dict(boxstyle="round",fc=(0.0, 0.0, 0.0), ec=(1., 1., 1.)), zorder=9)
        ...
        # newax = fig.add_axes([0.45, 0.0, 0.10, 0.10], anchor='S', zorder=12) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
        ...
        # Save the image
        # plt.savefig(out_dir + plot_config["file_name_id_1"] + "_" + plot_config["file_name_id_2"] + "_" + date_file + '.png', transparent=True)
        plt.savefig(out_dir + date_file + '.png', transparent=True)
