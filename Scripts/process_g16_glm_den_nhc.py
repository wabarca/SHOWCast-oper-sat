...
if (seconds == '00'):
    ...
    if (int(minutes) % acum_minutes == 0):
        ...
        colors = plt.get_cmap('cool')(range(256))
        ...
        "dpi": 150,
        "states_color": 'white', "states_width": 0.75,
        "countries_color": 'white', "countries_width": 0.75,
        "continents_color": 'white', "continents_width": 0.75,
        "grid_color": 'white', "grid_width": 0.3, "grid_interval": 5.0,
        "title_text": "GOES-19 GLM Descargas eléctricas + Banda 13 + NHC", "title_size": 7.5, "title_x_offset": int(sizex * 0.01), "title_y_offset": sizey - int(sizey * 0.016),
        ...
        img1 = ax.imshow(data, vmin=-80, vmax=40, cmap='Greys', alpha = 0.75, extent=img_extent, zorder=3)

        # Plot the GLM heatmap
        glm = ax.imshow(heatmap, vmin=0, vmax=100, interpolation='bilinear', extent=extent_glm, origin='lower', cmap=my_cmap, transform=ccrs.PlateCarree(), alpha = 0.75, zorder=4)
        ...
        img3 = ax.plot(g_lons,g_lats, 'o', markersize=0.5, markeredgecolor='blue', markerfacecolor='magenta', alpha = 0.2, transform=ccrs.PlateCarree(), zorder=5)
        ...
        ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["countries_color"],alpha = 0.3,facecolor='none', linewidth=plot_config["countries_width"], zorder=6)

        # Add continents
        shapefile = list(shpreader.Reader(main_dir + '//Shapefiles//GSHHS_h_L1_clipped.shp').geometries())
        ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["continents_color"],alpha = 0.3,facecolor='none', linewidth=plot_config["continents_width"], zorder=7)
        ...
        for name in tsystems:
            ...
            if storm.invest == True:
                ...
                # Add line trajectory
                if len(shp_files) > 0 and os.path.exists(shp_files[0]):
                    shapefile = list(shpreader.Reader(shp_files[0]).geometries())
                    ax.add_geometries(shapefile, ccrs.PlateCarree(),
                                    edgecolor='white', facecolor='none',
                                    linewidth=plot_config["states_width"]*4, zorder=7)

                # Add probabilities cone
                if len(shp_files) > 1 and os.path.exists(shp_files[1]):
                    shapefile = list(shpreader.Reader(shp_files[1]).geometries())
                    ax.add_geometries(shapefile, ccrs.PlateCarree(),
                                    edgecolor='black', facecolor='white', alpha=0.4,
                                    linewidth=plot_config["states_width"]*4, zorder=8)

                # Add positions
                if len(shp_files) > 2 and os.path.exists(shp_files[2]):
                    shapefile = list(shpreader.Reader(shp_files[2]).geometries())
                    ax.scatter([s.x for s in shapefile], [s.y for s in shapefile],
                    ...
                    for field in shapefile:
                        text = ax.text(field.attributes['LON'] + 0.5,
                                    field.attributes['LAT'] + 0.5,
                                    field.attributes['DVLBL'],
                                    color='blue', fontsize='large', fontweight='bold', zorder=10)
        ...
        newax = fig.add_axes([0.9, 0.0, 0.10, 0.10], anchor='S', zorder=15) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
        newax.imshow(my_logo)
        newax.axis('off')

        # Add nhc legend / images to the plot
        my_logo = plt.imread(main_dir + '//Logos//nhc_legend.webp')
        newax = fig.add_axes([0.0, 0.0, 0.15, 0.15], anchor='S', zorder=15) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
        newax.imshow(my_logo)
        newax.axis('off')
