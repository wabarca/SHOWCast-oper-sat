...
path_dbf = path2 + path3[: len(path3) - 8] + "0000.dbf"
path_shp_clip = path2 + path3[: len(path3) - 8] + "0000_clip.shp"
path_dbf_clip = path2 + path3[: len(path3) - 8] + "0000_clip.dbf"
path_csv_clip = path2 + path3[: len(path3) - 8] + "0000_clip.csv"
path_cpg_clip = path2 + path3[: len(path3) - 8] + "0000_clip.cpg"
path_prj_clip = path2 + path3[: len(path3) - 8] + "0000_clip.prj"
path_shx_clip = path2 + path3[: len(path3) - 8] + "0000_clip.shx"
...
# image = (main_dir + '//HTML//showcast_fire.webp')
image = "..//HTML//showcast_fire.webp"
FloatImage(image, bottom=7, left=1).add_to(m)

# image = (main_dir + '//Legends//showcast_fire_legend.webp')
# image = ('../Legends//showcast_fire_legend.webp')
...
shape = main_dir + "//Shapefiles//El_Salvador_departamentos.shp"
countries = gpd.read_file(shape)

shape = main_dir + "//Shapefiles//Centro_America.shp"
states = gpd.read_file(shape)

shape = main_dir + "//Shapefiles//GSHHS_h_L1_clipped.shp"
...
for lat, lng, label, time_date, country, state, city in zip(
    latitudes, longitudes, labels, times_dates, countries, states, cities
):
    if lng < -30:
        if label == "GOES-16":
            popup_text = (
            ...
            folium.Circle(
                ...
                weight=0.2,
                fill_opacity=1,
            ).add_child(folium.Popup(popup_text, max_width=250, min_width=250)).add_to(
                m
            )
            ...
            folium.Circle(
                ...
                weight=0.2,
                fill_opacity=1,
            ).add_child(folium.Popup(popup_text, max_width=200, min_width=200)).add_to(
                m
            )
...
# Adding refresh
file_path = main_dir + "//HTML//SHOWCast_Fire.html"
old_string = '<meta http-equiv="content-type" content="text/html; charset=UTF-8" />'
new_string = '<meta http-equiv="content-type" content="text/html; charset=UTF-8" /><meta http-equiv="refresh" content="600" >'
replace_string_in_file(file_path, old_string, new_string)
# Clipping the .shp to a small region
import fiona

with fiona.open(path_shape) as input:
    meta = input.meta
    with fiona.open(path_shp_clip, "w", **meta) as output:
        for feature in input:
            if (
                feature["properties"]["latitude"] <= 18.45
                and feature["properties"]["latitude"] >= 7
            ) and (
                feature["properties"]["longitude"] <= -77.0
                and feature["properties"]["longitude"] >= -92.15
            ):
                output.write(feature)
                output.close

# Convert the .dbf to .csv and move to internal server
import csv
...
def dbf_to_csv(path):
    ...
    with open(path_csv_clip, "w", newline="") as f:
        writer = csv.writer(f)
...
dbf_to_csv(path_dbf_clip)

# Copying .csv to data server
import subprocess

subprocess.run(
    ["scp", path_csv_clip, "ccpm@192.168.4.28:/var/www/html/gnca_fireshotspots/"]
)
subprocess.run(
    [
        "scp",
        path_cpg_clip,
        "ccpm@192.168.4.28:/var/www/html/gnca_fireshotspots/shapefiles/",
    ]
)
subprocess.run(
    [
...
# subprocess.Popen('rm *.csv', shell=True)
# ---------------------------------------------------------------------------------------------
# ---------------------------------------------------------------------------------------------
