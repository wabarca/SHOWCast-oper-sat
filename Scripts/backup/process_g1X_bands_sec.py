...
plot_config = {
    ...
    "grid_color": 'white', "grid_width": 0.3, 
    "vmin": vmin, "vmax": vmax, "cmap": cmap,
    "title_text": "GOES-" + satellite[1:3] + " Banda " + band + " (" + band_wavelength + " $\mu{m}$)", "title_size": 8, "title_x_offset": int(data.shape[1] * 0.01), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016), 
...
if sector == "_ATL":
    fig = plt.figure(figsize=(1920/float(plot_config["dpi"]), 785/float(plot_config["dpi"])), dpi=plot_config["dpi"])
else:
...
gl = ax.gridlines(color=plot_config["grid_color"], alpha=0.5, linestyle='--', linewidth=plot_config["grid_width"], xlocs=np.arange(-180, 180, grid_interval), ylocs=np.arange(-180, 180, grid_interval), draw_labels=True, zorder=5)
...
if sector == "_ATL":
    plt.annotate(plot_config["title_text"] + " " + date_formated , xy=(0.015,0.95), xycoords='figure fraction', fontsize=plot_config["title_size"], fontweight='bold', color='white', bbox=dict(boxstyle="round",fc=(0.0, 0.0, 0.0), ec=(1., 1., 1.)), zorder=6)
else:
...
if sector == '_ATL':
    newax = fig.add_axes([0.85, 0.05, 0.15, 0.15], anchor='SW', zorder=12) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
else:
...
cb.ax.tick_params(axis='x', colors='gainsboro', width=2, labelsize=plot_config["cbar_labelsize"])
#---------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------
