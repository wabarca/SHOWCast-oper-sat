...
for hour in range(hour_ini, hour_end, hour_inc):
    ...
    if (os.path.exists(path_loop)):
        ...
        "title_text": "GFS (0.5°): 2 m Temperature (°C)" + "    -    " + "Init: " + str(init)[:-6] + "Z | Forecast Hour: [" + str(ftime).zfill(3) + "]" + " | Valid: " + str(valid)[:-6] + "Z ", "title_size": 9, "title_x_offset": 1, "title_y_offset": 1, 
        "thick_interval": thick_interval, "cbar_labelsize": 6, "cbar_labelpad": -1,
        ...
        # Add gridlines
        gl = ax.gridlines(color=plot_config["grid_color"], alpha=1.0, linestyle='--', linewidth=plot_config["grid_width"], xlocs=np.arange(-180, 180, plot_config["grid_interval"]), ylocs=np.arange(-180, 180, plot_config["grid_interval"]), draw_labels=True, zorder=8)
        ...
        plt.annotate(plot_config["title_text"], xy=(0.015,0.965), xycoords='figure fraction', fontsize=plot_config["title_size"], fontweight='bold', color='white', bbox=dict(boxstyle="round",fc=(0.0, 0.0, 0.0), ec=(1., 1., 1.)), zorder=9)
        ...
        cb.set_label(label='2 m Temperature (°C)', size=plot_config["cbar_labelsize"]+1, weight='bold')
