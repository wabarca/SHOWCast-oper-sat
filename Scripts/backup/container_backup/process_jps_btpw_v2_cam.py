...
if (product == "TPW"):
    ...
    vmax = 80
    thick_interval = 5
    prod_title = "Blended Precipitable Water [mm]"
    #------------------------------------------------------------------------------------------------------
    #------------------------------------------------------------------------------------------------------
    # Plot configuration
    plot_config = {
    ...
    "dpi": 150, 
    "states_color": 'dimgray', "states_width": data.shape[0] * 0.005, 
    "countries_color": 'dimgray', "countries_width": data.shape[0] * 0.005,
    "continents_color": 'dimgray', "continents_width": data.shape[0] * 0.005,
    "grid_color": 'white', "grid_width": data.shape[0] * 0.005, "grid_interval": 5.0,
    "vmin": vmin, "vmax": vmax, "cmap": cmap,
    "title_text": "Multimission " + prod_title , "title_size": int(data.shape[1] * 0.04), "title_x_offset": int(data.shape[1] * 0.01), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.032), 
    "thick_interval": thick_interval, "cbar_labelsize": int(data.shape[0] * 0.08), "cbar_labelpad": -int(data.shape[0] * 0.0),
    "file_name_id_1": "MUL",  "file_name_id_2": product + "_CAM"
    }
elif (product == "PCT"):
    product = "BPCT"
    ...
    vmax = 300
    thick_interval = 20
    prod_title = "Precipitable Water Anomaly [mm]"
    #------------------------------------------------------------------------------------------------------
    #------------------------------------------------------------------------------------------------------
    # Plot configuration
    plot_config = {
    ...
    vmax = 10
    thick_interval = 0.5
    prod_title = "Blended Rain Rate [mm/h]"
    data[data == 0] = np.nan
        #------------------------------------------------------------------------------------------------------
    #------------------------------------------------------------------------------------------------------
    # Plot configuration
...
# Plot the image
data = np.flipud(data)
img = ax.imshow(data, vmin=plot_config["vmin"], vmax=plot_config["vmax"], origin='upper', extent=img_extent, cmap=plot_config["cmap"], interpolation='spline36', zorder=3)
