...
if (seconds == '00'):
    ...
    if (int(minutes) % acum_minutes == 0):
        ...
        "dpi": 150, 
        "states_color": 'white', "states_width": sizey * 0.00025, 
        "countries_color": 'white', "countries_width": sizey * 0.00025,
        "continents_color": 'white', "continents_width": sizey * 0.00025,
        "grid_color": 'white', "grid_width": sizey * 0.00025, "grid_interval": 5.0,
        "title_text": "GOES-16 GLM (Density) + Band 13 + NHC", "title_size": 7.5, "title_x_offset": int(sizex * 0.01), "title_y_offset": sizey - int(sizey * 0.016), 
        ...
        img1 = ax.imshow(data, vmin=-80, vmax=40, cmap='Greys', alpha = 0.750, extent=img_extent, zorder=3) 

        # Plot the GLM heatmap
        glm = ax.imshow(heatmap, vmin=0, vmax=100, interpolation='bilinear', extent=extent_glm, origin='lower', cmap=my_cmap, transform=ccrs.PlateCarree(), alpha = 0.9, zorder=4)
        ...
        img3 = ax.plot(g_lons,g_lats, 'o', markersize=1.0, markeredgecolor='blue', markerfacecolor='magenta', alpha = 0.2, transform=ccrs.PlateCarree(), zorder=5)
        ...
        areas_medium = [x.geometry for x in shapefile if float(x.attributes['PROB7DAY'].replace("%", "")) >= 40 and float(x.attributes['PROB2DAY'].replace("%", "")) <= 60]
        ...
        trajectories_medium = [x.geometry for x in shapefile if float(x.attributes['PROB7DAY'].replace("%", "")) >= 40 and float(x.attributes['PROB2DAY'].replace("%", "")) <= 60]
        ...
        points_medium = [x for x in shapefile if float(x.attributes['PROB7DAY'].replace("%", "")) >= 40 and float(x.attributes['PROB2DAY'].replace("%", "")) <= 60]
        ...
        newax = fig.add_axes([0.01, 0.04, 0.10, 0.10], anchor='SW', zorder=15) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
