...
# Read the units
variable_units = str(file.variables['chlor_a'].units)
...
date = datetime(1970,1,1,12) + timedelta(seconds=add_seconds) - timedelta(days=1)
...
vmax = 65.00
thick_interval = 1.0
prod_title = 'NOAA-20 - VIIRS Concentración de clorofila-a (Chl-a)'
satellite = 'JPS'
product = 'OCRG' + region
...
"title_text": prod_title + " ["+variable_units+"] ", "title_size": 8, "title_x_offset": int(sizex * 0.01), "title_y_offset": sizey - int(sizey * 0.016),
...
ax.imshow(imread(fname), origin='upper', transform=ccrs.PlateCarree(), extent=[-92.930, -83.803, 10.78, 15.750], zorder=1)#date = datetime(int(year), int(month), int(day), int(hour))
...
img = ax.imshow(data, norm=norm, origin='upper', extent=img_extent, cmap=plot_config["cmap"], zorder=3)
...
cb.set_label(label='Concentración de clorofila-a (mg/m³)', color='white', size=plot_config["title_size"], weight='bold')
cb.outline.set_visible(False)
cb.ax.tick_params(width = 1)
cb.ax.xaxis.set_tick_params(pad=plot_config["cbar_labelpad"])
cb.ax.xaxis.set_ticks_position('top')
cb.ax.xaxis.set_label_position('top')
cb.ax.tick_params(axis='x', which='both', colors='white', labelsize=plot_config["cbar_labelsize"])
