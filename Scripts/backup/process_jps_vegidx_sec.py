...
if (prod == 'NDA'):
    ...
    prod_title = 'JPSS VIIRS NDVI en el Tope de la Atmósfera (TOA) 4km'
    label = 'NDVI en el Tope de la Atmósfera (TOA)'
    satellite = 'JPS'
    product = 'NDVITA'
if (prod == 'NDC'):
    # Extract the data
    data = file.variables['NDVI_TOC'][ latui:latli , lonli:lonui ]
    prod_title = 'JPSS VIIRS NDVI en el Tope del Dosel Arbóreo (TOC) 4km'
    label = 'NDVI en el Tope del Dosel arbóreo (TOC)'
    satellite = 'JPS'
    product = 'NDVITC'
if (prod == 'EVC'):
    # Extract the data
    data = file.variables['EVI_TOC'][ latui:latli , lonli:lonui ]
    prod_title = 'JPSS VIIRS Índice de Vegetación Mejorado en el Tope del Dosel Arbóreo (TOC) 4km'
    label = 'Índice de Vegetación Mejorado en el Tope del Dosel Arbóreo (TOC)'
    satellite = 'JPS'
    product = 'EVITOC'
...
gl = ax.gridlines(color=plot_config["grid_color"], alpha=1.0, linestyle='--', linewidth=plot_config["grid_width"], xlocs=np.arange(-180, 180, plot_config["grid_interval"]), ylocs=np.arange(-180, 180, plot_config["grid_interval"]), draw_labels=True, zorder=7)
...
cb.set_label(label=label, color='white', size=plot_config["title_size"], weight='bold')
cb.outline.set_visible(False)
cb.ax.tick_params(width = 0)
cb.ax.xaxis.set_tick_params(pad=plot_config["cbar_labelpad"])
cb.ax.xaxis.set_ticks_position('top')
cb.ax.xaxis.set_label_position('top')
cb.ax.tick_params(axis='x', colors='white', labelsize=plot_config["cbar_labelsize"])
