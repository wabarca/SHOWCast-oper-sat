...
osystem = platform.system()
if osystem == "Windows":
    path_ch02 = re.sub('Band07\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch07)
else:
    path_ch02 = re.sub('Band07/OR_ABI-L2-CMIPF*(.*)', '', path_ch07)
...
data_ch07 = (data_ch07)/(np.cos(np.deg2rad(sun_zenith)))
data_ch03 = (data_ch03)/(np.cos(np.deg2rad(sun_zenith)))
data_ch02 = (data_ch02)/(np.cos(np.deg2rad(sun_zenith)))
data_ch13 = (data_ch13)/(np.cos(np.deg2rad(sun_zenith)))


# Calculate the solar component (3.9 um band)
from pyspectral.near_infrared_reflectance import Calculator
refl39 = Calculator('GOES-16', 'abi', 'ch7')
data_ch07_refl = refl39.reflectance_from_tbs(sun_zenith, data_ch07,data_ch13)
print("Solar Component calculus finished")

data_ch07_refl = (data_ch07_refl)/(np.cos(np.deg2rad(sun_zenith)))
...
plot_config = {
    ...
    "title_text": "GOES-" + satellite[1:3] + " Day Land Cloud Fire RGB (Detección de nubes y fuegos)", "title_size": 8, "title_x_offset": 8, "title_y_offset": data_ch07.shape[0] - int(data_ch07.shape[0] * 0.016), 
    "thick_interval": 0, "cbar_labelsize": 8, "cbar_labelpad": -int(data_ch07.shape[0] * 0.0),
...
my_legend = plt.imread(main_dir + '//Legends//DAYLANDCLOUDFIRE_legend.webp')
