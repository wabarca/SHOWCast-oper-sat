...
"thick_interval": thick_interval, "cbar_labelsize": int(data.shape[0] * 0.007), "cbar_labelpad": -int(data.shape[0] * 0.0),
...
# Add a title
plt.annotate(plot_config["title_text"] + " " + date_formated , xy=(0.015,0.96), xycoords='figure fraction', fontsize=plot_config["title_size"], fontweight='bold', color='white', bbox=dict(boxstyle="round",fc=(0.0, 0.0, 0.0), ec=(1., 1., 1.)), zorder=8)
...
my_logo = plt.imread(main_dir + '//Logos//noaa_logo.webp')
newax = fig.add_axes([0.12, 0.04, 0.08, 0.08], anchor='SW', zorder=12) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
newax.imshow(my_logo)
newax.axis('off')

#my_logo = plt.imread(main_dir + '//Logos//gmu_logo.webp')
...
my_legend = plt.imread(main_dir + '//Legends//FLOOD_legend.webp')
