...
if (seconds == '00'):
    ...
    if (int(minutes) % acum_minutes == 0):
        ...
        "title_text": "GOES-16 GLM Descargas Eléctricas (Seguimiento) + Banda "+band, "title_size": 8, "title_x_offset": int(sizex * 0.01), "title_y_offset": sizey - int(sizey * 0.016), 
        ...
        img1 = ax.imshow(data, vmin=-80, vmax=40, cmap='Greys', alpha = 0.5, extent=img_extent, zorder=3) 
        ...
        # img3a = ax.plot(e3_lons,e3_lats, 'o', markersize=2.0, color='lime', markerfacecolor='lime', alpha = 1.00, transform=ccrs.PlateCarree(), zorder=5)
        img3b = ax.plot(g3_lons,g3_lats, '.', markersize=3.0, color='lime', markerfacecolor='lime', alpha = 1.00, transform=ccrs.PlateCarree(), zorder=4)
        ...
        # Plot the accumulated flashes, groups and events
        # img2a = ax.plot(e2_lons,e2_lats, 'o', markersize=2.0, color='gold', markerfacecolor='gold', alpha = 1.00, transform=ccrs.PlateCarree(), zorder=9)
        img2b = ax.plot(g2_lons,g2_lats, '.', markersize=3.0, color='yellow', markerfacecolor='yellow', alpha = 1.00, transform=ccrs.PlateCarree(), zorder=5)
        ...
        # Plot the accumulated flashes, groups and events
        # img1a = ax.plot(e1_lons,e1_lats, 'o', markersize=2.0, color='red', markerfacecolor='red', alpha = 1.00, transform=ccrs.PlateCarree(), zorder=13)
        img1b = ax.plot(g1_lons,g1_lats, '.', markersize=3.0, color='red', markerfacecolor='red', alpha = 1.00, transform=ccrs.PlateCarree(), zorder=6)
        ...
        ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["states_color"],alpha = 0.3,facecolor='none', linewidth=plot_config["states_width"], zorder=7)

        # Add countries
        shapefile = list(shpreader.Reader(main_dir + '//Shapefiles//Centro_America.shp').geometries())
        ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["countries_color"],alpha = 0.3,facecolor='none', linewidth=plot_config["countries_width"], zorder=8)
        ...
        # Add gridlines
        gl = ax.gridlines(color=plot_config["grid_color"], alpha=0.3, linestyle='--', linewidth=plot_config["grid_width"], xlocs=np.arange(-180, 180, plot_config["grid_interval"]), ylocs=np.arange(-180, 180, plot_config["grid_interval"]), draw_labels=True, zorder=9)
        ...
        newax = fig.add_axes([0.84, 0.05, 0.15, 0.15], anchor='SW', zorder=13) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
        newax.imshow(my_logo)
        newax.axis('off')

        # Add a legend to the plot
        my_legend = plt.imread(main_dir + '//Legends//GLM_TRACK_legend.webp')
        newax = fig.add_axes([0.87, 0.87, 0.12, 0.12], anchor='NE', zorder=14) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
