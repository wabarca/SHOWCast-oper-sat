...
# Read the units
variable_units = str(nc.variables[product].units)
if (variable_units=='Percent'):
   variable_units = '%'
...
if (product == "TPW"):
    ...
    colors = ["#3f3220", "#956f48", "#d7a678", "#9393f7", "#07626a",
            "#f0f507", "#fa8362", "#740070", "#f62df4", "#f8b4fa"]
    cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", colors)
    cmap.set_over('#f8b4fa')
    cmap.set_under('#3f3220')
    vmin=0
    vmax=80
    thick_interval = 5
    prod_title = "Agua Precipitable Total (Blended TPW)"
elif (product == "PCT"):
    product = "BPCT"
    ...
    vmax = 250
    thick_interval = 20
    prod_title = "Anomalía de Agua Precipitable Total (Blended PCT)"
elif (product == "RR"):
    product = "BRRT"
...
"title_text": "Multimission - " + prod_title +" ["+variable_units+"]", "title_size": 8, "title_x_offset": int(data.shape[1] * 0.01), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.032), 
"thick_interval": thick_interval, "cbar_labelsize": 8, "cbar_labelpad": -int(data.shape[0] * 0.0),
"file_name_id_1": "MUL",  "file_name_id_2": product + sector
}
#------------------------------------------------------------------------------------------------------
...
# Plot the image
data = np.flipud(data)
img = ax.imshow(data, vmin=plot_config["vmin"], vmax=plot_config["vmax"], origin='upper', extent=img_extent, cmap=plot_config["cmap"], interpolation='bicubic', zorder=1)
...
if sector == '_SLV':
    ...
    ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["states_color"],alpha=0.5,facecolor='none', linewidth=plot_config["states_width"], zorder=2)

    # Add countries
    shapefile = list(shpreader.Reader(main_dir + '//Shapefiles//Centro_America.shp').geometries())
    ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["countries_color"],alpha=0.5,facecolor='none', linewidth=plot_config["countries_width"], zorder=3)
else:
    grid_interval = 5.0
    # Add countries
    shapefile = list(shpreader.Reader(main_dir + '//Shapefiles//Centro_America.shp').geometries())
    ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["countries_color"],alpha=0.5,facecolor='none', linewidth=plot_config["countries_width"], zorder=2)

    # Add continents
    shapefile = list(shpreader.Reader(main_dir + '//Shapefiles//GSHHS_h_L1_clipped.shp').geometries())
    ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["continents_color"],alpha=0.5,facecolor='none', linewidth=plot_config["continents_width"], zorder=3)

# Add coastlines, borders and gridlines
gl = ax.gridlines(color=plot_config["grid_color"], alpha=0.5, linestyle='--', linewidth=plot_config["grid_width"], xlocs=np.arange(-180, 180, grid_interval), ylocs=np.arange(-180, 180, grid_interval), draw_labels=True, zorder=7)
...
# Add a title
plt.annotate(plot_config["title_text"] + " " + date_formated , xy=(0.015,0.965), xycoords='figure fraction', fontsize=plot_config["title_size"], fontweight='bold', color='white', bbox=dict(boxstyle="round",fc=(0.0, 0.0, 0.0), ec=(1., 1., 1.)), zorder=5)
...
newax = fig.add_axes([0.84, 0.05, 0.15, 0.15], anchor='SW', zorder=6) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
...
cb.set_label(label=prod_title +' ['+variable_units+']', color='black', size=plot_config["title_size"], weight='bold')
