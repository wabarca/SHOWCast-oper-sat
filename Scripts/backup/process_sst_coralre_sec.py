...
if sector== "_SLV":
   ...
   # Configuration 
   # colors = ["#0000a1", "#0000fe", "#0034ff", "#0356fc", "#0199fc", "#01daff", "#238e25", "#00a001", "#4bcc4e", "#62fe64", "#f5fe81", "#ffff02", "#ffb200", "#fd5106", "#ff2600", "#fb6a94", "#b52c64"]
   colors = ["#690011", "#540246", "#6f0066", "#8e0184", "#2d0798", "#4b2eac", "#6959c2", "#00009e", "#001ef9", "#0053fe", "#0089fe", "#00b1fd", "#00e5ff", "#02f800", "#00cf01","#00a300", "#e7f000", "#e7b100", "#e67b00", "#dc4d00", "#c40f00", "#7d0000","#bc602d", "#8f4824", "#5c2f17"]
   # cmap = matplotlib.colors.ListedColormap(colors)
   cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", colors)
   cmap.set_over('#2f0114')
   cmap.set_under('#32190d')
...
if sector=="_SLV":
   ...
   ax.imshow(imread(fname), origin='upper', transform=ccrs.PlateCarree(), extent=[-92.930, -83.803, 10.78, 15.750], zorder=1)
else:
...
if sector=='_ATL':
   # Add a title
   plt.annotate(plot_config["title_text"] + " " + date_formated , xy=(0.015,0.95), xycoords='figure fraction', fontsize=plot_config["title_size"], fontweight='bold', color='white', bbox=dict(boxstyle="round",fc=(0.0, 0.0, 0.0), ec=(1., 1., 1.)), zorder=5) 
else:
   # Add a title
...
gl = ax.gridlines(color=plot_config["grid_color"], alpha=0.25, linestyle='--', linewidth=plot_config["grid_width"], xlocs=np.arange(-180, 180, grid_interval), ylocs=np.arange(-180, 180, grid_interval), draw_labels=True, zorder=6)
...
newax = fig.add_axes([0.84, 0.05, 0.15, 0.15], anchor='SW', zorder=7) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
