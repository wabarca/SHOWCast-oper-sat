...
# Band 13 file
path_ch12 = (sys.argv[1])
#path_ch12 = ("..//Samples//OR_ABI-L2-CMIPF-M6C13_G16_s20192931650344_e20192931700064_c20192931700142.nc")
#path_ch12 = ("..//Samples//OR_ABI-L2-CMIPF-M6C13_G17_s20192931650341_e20192931659418_c20192931659465-132020_0.nc")
...
path_ch12 = path_ch12[:-7]

# Get the Band 15, same file
osystem = platform.system()
if osystem == "Windows":
    path_ch15 = re.sub('Band12\\\\OR_ABI-L2-CMIPF*(.*)', '', path_ch12)
else:
    path_ch15 = re.sub('Band12/OR_ABI-L2-CMIPF*(.*)', '', path_ch12)

# Get the start of scan from the file name
scan_start = (path_ch12[path_ch12.find("_s")+2:path_ch12.find("_e")])
...
# Read the satellite 
satellite = getattr(file_ch12, 'platform_ID')

# Read the band number
band = str(file_ch12.variables['band_id'][0]).zfill(2)
...
longitude = file_ch12.variables['goes_imager_projection'].longitude_of_projection_origin

# Read the semi major axis
a = file_ch12.variables['goes_imager_projection'].semi_major_axis

# Read the semi minor axis
b = file_ch12.variables['goes_imager_projection'].semi_minor_axis

# Calculate the image extent 
h = file_ch12.variables['goes_imager_projection'].perspective_point_height
x1 = file_ch12.variables['x_image_bounds'][0] * h 
x2 = file_ch12.variables['x_image_bounds'][1] * h 
y1 = file_ch12.variables['y_image_bounds'][1] * h 
y2 = file_ch12.variables['y_image_bounds'][0] * h 

# Getting the file time and date
add_seconds = int(file_ch12.variables['time_bounds'][0])
...
#------------------------------------------------------------------------------------------------------
# Create the SWD
data = data_ch12 - data_ch15
#------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------
# SWD    
colors = ["#000000","#824513","#8d5220","#c09263","#f4d2a6","#fff9f2","#0404fa","#fcfc02","#ff6600","#ff00eb","#ff00ff"]
...
"title_text": "GOES-" + satellite[1:3] + " Split Window Difference (Diferencia de bandas 12-15) ", "title_size": 8, "title_x_offset": int(data.shape[1] * 0.01), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016), 
"thick_interval": thick_interval, "cbar_labelsize": 7, "cbar_labelpad": -int(data.shape[0] * 0),
