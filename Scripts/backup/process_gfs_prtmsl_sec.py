...
for hour in range(hour_ini, hour_end, hour_inc):
    ...
    if (os.path.exists(path_loop)):
        ...
        img3 = ax.barbs(lons[::6,::6], lats[::6,::6], ucomp[::6,::6], vcomp[::6,::6], length = 5.0, sizes = dict(emptybarb=0.0, spacing=0.2, height=0.5), linewidth=0.5, pivot='middle', barbcolor='gray', transform=ccrs.PlateCarree(), zorder=4)
        ...
        # Add gridlines
        gl = ax.gridlines(color=plot_config["grid_color"], alpha=1.0, linestyle='--', linewidth=0.25, xlocs=np.arange(-180, 180, 5), ylocs=np.arange(-180, 180, 5), draw_labels=True, zorder=8)
