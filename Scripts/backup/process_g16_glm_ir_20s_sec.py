...
"title_text": "GOES-16 GLM Descargas Eléctricas (20s) + Banda "+band, "title_size": 8, "title_x_offset": int(data.shape[1] * 0.01), "title_y_offset": data.shape[0] - int(data.shape[0] * 0.016), 
...
img1 = ax.imshow(data, vmin=-100, vmax=40, cmap='Greys', alpha = 0.5, extent=img_extent, zorder=3) 
...
# img2 = ax.plot(e_lons,e_lats, 'o', markersize=2.5, color='#0019f4', alpha = 0.5, transform=ccrs.PlateCarree(), zorder=4)
img3 = ax.plot(g_lons,g_lats, '.', markersize=3,color='yellow',alpha = 1.0, transform=ccrs.PlateCarree(), zorder=5)
# img4 = ax.plot(f_lons,f_lats, 'o', markersize=2.5, color='#0019f4', alpha = 0.5, transform=ccrs.PlateCarree(), zorder=6)
#---------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------
...
# ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["continents_color"],alpha = 0.3,facecolor='none', linewidth=plot_config["continents_width"], zorder=9)
  
# Add gridlines
gl = ax.gridlines(color=plot_config["grid_color"], alpha=0.3, linestyle='--', linewidth=plot_config["grid_width"], xlocs=np.arange(-180, 180, plot_config["grid_interval"]), ylocs=np.arange(-180, 180, plot_config["grid_interval"]), draw_labels=True, zorder=10)
...
newax = fig.add_axes([0.84, 0.015, 0.15, 0.15], anchor='SW', zorder=12) #  [left, bottom, width, height]. All quantities are in fractions of figure width and height.
