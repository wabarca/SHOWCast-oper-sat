...
if (seconds == '00'):
    ...
    if (int(minutes) % acum_minutes == 0):
        ...
        "title_text": "GOES-16 GLM Descargas Eléctricas - Densidad ("+str(acum_minutes)+" min) + Banda "+band, "title_size": 8, "title_x_offset": int(sizex * 0.01), "title_y_offset": sizey - int(sizey * 0.016), 
        ...
        glm = ax.imshow(heatmap, vmin=0, vmax=60, interpolation='bilinear', extent=extent_glm, origin='lower', cmap=my_cmap, transform=ccrs.PlateCarree(), alpha = 0.75, zorder=4)

        # Plot the accumulated flashes, groups and events
        # img2 = ax.plot(e_lons,e_lats, 'o', markersize=2.0, color='#fffb8a', alpha = 0.01, transform=ccrs.PlateCarree(), zorder=5)
        img3 = ax.plot(g_lons,g_lats, '.', markersize=3.0, color='yellow', alpha = 1.0, transform=ccrs.PlateCarree(), zorder=5)
        # img4 = ax.plot(f_lons,f_lats, 'o', markersize=2.0, color='#fff700', alpha = 0.20, transform=ccrs.PlateCarree(), zorder=7)
        ...
        ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["states_color"],alpha = 0.3,facecolor='none', linewidth=plot_config["states_width"], zorder=6)

        # Add countries
        shapefile = list(shpreader.Reader(main_dir + '//Shapefiles//Centro_America.shp').geometries())
        ax.add_geometries(shapefile, ccrs.PlateCarree(), edgecolor=plot_config["countries_color"],alpha = 0.3,facecolor='none', linewidth=plot_config["countries_width"], zorder=7)
