...
for hour in range(hour_ini, hour_end, hour_inc):
    ...
    if (os.path.exists(path_loop)):
        ...
        colors = [
        '#ADD8E6', '#87CEEB', '#6495ED', '#4682B4', '#4169E1',  # Light Blue to Blue
        '#00FF00', '#7FFF00', '#ADFF2F', '#FFFF00',             # Green to Yellow
        '#FFD700', '#FFA500', '#FF7F50', '#FF6347', '#FF4500',  # Orange to Red
        '#DC143C', '#800000',                                    # Red to Dark Red
        '#800080', '#9370DB', '#8A2BE2', '#4B0082'              # Purple
        ]
        ...
        "countries_color": 'black', "countries_width": 0.4,
        "continents_color": 'black', "continents_width": 0.4,
        "grid_color": 'white', "grid_width": 0.3, "grid_interval": 5.0,
        "vmin": vmin, "vmax": vmax, "cmap": cmap,
        "title_text": "GFS (0.5°): Total Precipitation" + "    -    " + "Init: " + str(init)[:-6] + "Z | Forecast Hour: [" + str(ftime).zfill(3) + "]" + " | Valid: " + str(valid)[:-6] + "Z ", "title_size": 9, "title_x_offset": 1, "title_y_offset": 1,
        "thick_interval": thick_interval, "cbar_labelsize": 7, "cbar_labelpad": -1,
        ...
        img3 = ax.contourf(lons, lats, totpr, transform=ccrs.PlateCarree(), cmap=cmap2, levels=levels2, zorder=2,alpha=0.6)
        ...
        img1 = ax.contourf(lons, lats, totpr, transform=ccrs.PlateCarree(), cmap=cmap, levels=levels, extend='max', zorder=3,alpha=0.6)
        img2 = ax.contour(lons, lats, totpr, transform=ccrs.PlateCarree(), colors='dimgrey', linewidths=0.1, levels=levels, zorder=4,alpha=0.75)
        ax.clabel(img2, inline=1, inline_spacing=0, fontsize=plot_config["cbar_labelsize"],fmt = '%1.0f', colors= 'mediumblue', zorder=5)
        ...
        # Add gridlines
        gl = ax.gridlines(color=plot_config["grid_color"], alpha=1.0, linestyle='--', linewidth=0.25, xlocs=np.arange(-180, 180, 5), ylocs=np.arange(-180, 180, 5), draw_labels=True, zorder=9)
        ...
        # Add a title
        plt.annotate(plot_config["title_text"], xy=(0.015,0.965), xycoords='figure fraction', fontsize=plot_config["title_size"], fontweight='bold', color='white', bbox=dict(boxstyle="round",fc=(0.0, 0.0, 0.0), ec=(1., 1., 1.)), zorder=10)
