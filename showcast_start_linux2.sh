...
if [ -d "$(pwd)/Miniconda3/envs/showcast2" ]; then

	# echo "Calling showcast_cleaner.py"
	# gnome-terminal --tab -e 'bash -c "$(pwd)/Miniconda3/envs/showcast2/bin/python Scripts/showcast_cleaner.py; exec bash"' 

	echo "Calling showcast_start.py"
	for i in $( seq 1 $num_process)
	do
		#gnome-terminal --tab -e 'bash -c "echo $(pwd)/Miniconda3/envs/showcast2/bin/python Scripts/showcast_start.py '$i'; exec bash"' 
		gnome-terminal --tab -e 'bash -c "$(pwd)/Miniconda3/envs/showcast2/bin/python Scripts/showcast_start2.py '$i'; exec bash"' 
	done	
